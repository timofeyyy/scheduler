...
from ..db_oim_orm import Capacitors


class CapacitorsRepo(ComponentBase, ComponentFamilyBase):
    __typename__ = Capacitors.__typename__

    def select_all_wrap(self):
        return self.select_all(table=Capacitors)
    ...
    def insert_many(self, rows):
        ...
        for row in rows:
            ...
            if existing_rows is not None and compare_rows(row=row, existing_rows=existing_rows, component_type=Capacitors):
                continue
            ...
            kind_id = self.kind.get_obj_by_ru_value("smd-конденсаторы").ID
    ...
    #         return db.query(Capacitors).all()
