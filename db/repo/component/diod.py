...
from ..db_oim_orm import Diods


class DiodsRepo(ComponentBase, ComponentFamilyBase):
    __typename__ = Diods.__typename__

    def select_all_wrap(self):
        return self.select_all(table=Diods)
    ...
    def insert_many(self, rows):
        ...
        for row in rows:
            ...
            if existing_rows is not None and compare_rows(row=row, existing_rows=existing_rows, component_type=Diods):
                continue
            ...
            kind_id = self.kind.get_obj_by_ru_value("диоды силовые").ID
