...
from .family import ComponentBase, make_dictionary, compare_rows, ComponentFamilyBase
from scheduler.settings import get_settings
from ..db_oim_orm import Transistors


class TransistorsRepo(ComponentBase, ComponentFamilyBase):
    __typename__ = Transistors.__typename__

    def select_all_wrap(self):
        return self.select_all(table=Transistors)
    ...
    def insert_many(self, rows):
        ...
        for row in rows:
            ...
            if existing_rows is not None and compare_rows(row=row, existing_rows=existing_rows, component_type=Transistors):
                continue
            ...
            kind_id = self.kind.get_obj_by_ru_value("биполярные транзисторы").ID
    ...
    #         return db.query(Transistors).all()
